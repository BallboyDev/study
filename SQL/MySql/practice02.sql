...
select max(salary), min(salary) from salaries where to_date > curdate();

-- 문제6.
-- 최고 어린 사원의 나이와 최 연장자의 나이는?
select period_diff(date_format(max(birth_date), '%Y%m'), date_format(min(birth_date), '%Y%m')) from employees;
