...
select gender, count(*) as "성별 인원수" from employees group by gender;
...
select count(*) from dept_manager where to_date > curdate();
...
select title, count(title) from titles group by title having title = "Engineer";
